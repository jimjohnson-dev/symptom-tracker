...
public interface IEnvironmentSnapshotRepository
{
    ...
    /// <summary>
    /// Returns the snapshot with the most recent creation time before <param name="createdAt"></param>.
    /// Avoids table scanning by calculating pressure deltas at write time.
    /// </summary>
    Task<EnvironmentSnapshot?> GetMostRecentBeforeAsync(DateTime createdAt, CancellationToken cancellationToken = default);
